package com.rquest.test.ftpclient;
...
public class FtpUtils {
    ...
    public static void main(String[] args) {
        FtpUtils f = new FtpUtils("192.168.22.165", 21, "user1", "user1");
        try {
            if(f.open()) {
                String fileName = "测试2.xml";
                //上传
                f.upload("d:/a.xml", fileName, "03开发共享");

                //遍历
                FTPFile[] list = f.getFileList("03开发共享");
                ...
                //只遍历指定目录下的文件名
                String[] names = f.getFileNameList("03开发共享");
                for(String name : names) {
                    System.out.println(new String(name.getBytes("iso-8859-1"), "GB2312"));
                }

                //下载
                boolean b = f.get("/03开发共享/ysa.jar", "d:/ysa.jar");
